...
async def main():
    ...
    try:
        ...
        print("❌ gemini --acp가 JSON-RPC 응답을 반환하지 않습니다.")
        print("   gemini-cli는 ACP 모드를 지원하지 않을 수 있습니다.")
        proc.terminate(); await proc.wait()
        return
    ...
    try:
        line = await asyncio.wait_for(proc.stdout.readline(), timeout=5)
        session_resp = json.loads(line.decode().strip())
