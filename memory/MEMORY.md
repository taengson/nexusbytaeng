...
| :--- | :--- | :--- | :--- |
| **Main Entry** | `src/main.py` | [완성] 메인 앱 루프 및 스크린 라우팅 | `HomeScreen` ↔️ `#workspace-container` 토글 처리 |
| **Home Screen** | `src/screens/home_screen.py` | [완성] 4개 AI/Network 연결 진입 허브 | 복귀 시 세션 작업 영역 자동 숨김 보장 |
| **Project Tree** | `src/widgets/project_tree.py` | [완성] 워크스페이스 내 프로젝트 소스 트리 | 탭 활성화 시에만 노출되도록 격리 |
| **Input Engine** | `src/core/input_engine.py` | [진행] `InputDispatcher` 등 파싱/검증 로직 | 실시간 `get_suggestions(text)` 제안 기능 고도화 필요 |
| **Input Area** | `src/widgets/input_area.py` | [진행] 모드별 테마 및 제안 패널 연동 UI | `on_change` 이벤트 연동 및 `.shell-bubble` 적용 대기 |
| **Suggestion UI**| `src/widgets/suggestions.py` | [진행] 추천 키워드 리스트 뷰 패널 | 패널 상단 안내 가이드 `Label` 추가 대기 |
| **Global Style** | `src/styles/nexus.tcss` | [진행] 다크 다이얼 테마 및 카드 메시지 버블 | 모달 창 컷팅 현상 방지 및 모드별 색상 튜닝 중 |
| **Global State** | `src/core/state.py` | [스켈레톤] 모드 및 액티브 세션 상태 제어 | 추후 세션 데이터 직렬화 시 확장 필요 |
...
AI 에이전트는 작업을 할당받을 때마다 아래의 **4단계 라이프사이클**을 강제 준수해야 합니다.

```
 [1. Read Stage] ──➡️ [2. Code Stage] ──➡️ [3. Validate Stage] ──➡️ [4. Update Stage]
   - 마스터 리드          - 규칙 내 surgical edit      - 실행 및 테스트 검증        - 마크다운 메모리 갱신
   - 상태 정렬            - 안티패턴 방지              - 빌드/타입 에러 체크        - 인덱스 일지 추가
```
